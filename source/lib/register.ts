import { createRequire, registerHooks, register } from 'node:module'
...
if (major >= 25 || (major === 24 && minor >= 15)) {
    ...
    // Also patch the cjs loader if Node < 26.2
    // (https://github.com/nodejs/node/pull/62920 landed in 26.2)
    if (major < 26 || (major === 26 && minor < 2))
        patchCjsLoader()
}
else {
    // For older Node.js, use the legacy `register()` API for esm and patch the cjs loader.
